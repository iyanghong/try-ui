        ...
        <template v-for="(group,index) in data">
            <template v-if="group.children">
                <div class="nav-group" :key="index">
                    ...
                    <div v-for="(nav,navIndex) in group.children" :key="navIndex" class="nav-item">
                        <a :class="{'active':isActive(nav.path)}" :href="getComponentPagePath(nav.path)">
            ...
            <template v-else>
                <div class="nav-item" :key="index">
                    <a :class="{'active':isActive(group.path)}" :href="getComponentPagePath(group.path)">{{
                            group.title
...
export default {
    ...
    methods: {
        ...
        getComponentPagePath(path) {
            return `${process.env.PUBLIC_PATH || '/'}components${path}`;
        }
    }
}
</script>
